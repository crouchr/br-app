...
def writeFlow2SEC(line):
    ...
    try:          
        ...
        syslog.syslog("kojoney_tail.py : writeFlow2SEC() exception caught = " + repr(e))
...
def sendTweetCLI(sessionid,username,ip,cli):
    ...
    try:
        ...
        msg = "kojoney,mars,USER,ssh,sid=" + repr(sessionid) + ",IP=" + ip + "," + asNum + "(" + asRegisteredCode + ")" + ","\
...
def writeSecVizSebek(uid,pid,ip,countryCode,cmd,cmdStr):
    ...
    try:    
        ...
        syslog.syslog("kojoney_tail.py : writeSecVizSebek() exception caught = " + repr(e))
...
def processNepenthes(line):
    try:
        ...
        syslog.syslog("kojoney_tail.py : processNepenthes() exception caught = " + repr(e) + " line=" + line.strip("\n"))
        return None
...
def processFWsnort(line):
    ...
    try:        
      ...
      # Anonymise/compress router (node9) honeypot IP address
        ...
        syslog.syslog("kojoney_tail.py : processFWsnort() exception caught = " + repr(e) + "line=" + line)
...
def processrtr(line):
    ...
    try:        
        ...
        syslog.syslog("kojoney_tail.py : processrtr() exception caught = " + repr(e) + "line=" + line)
...
def processaaa(line):
    ...
    try:            
        ...
        syslog.syslog("kojoney_tail.py : processaaa() [authentication] exception caught = " + repr(e) + "line=" + line)
...
def processaaaa(line):
    ...
    try:            
        ...
        syslog.syslog("kojoney_tail.py : processaaaa() [accounting] exception caught = " + repr(e) + "line=" + line)
...
def processdns(line):
    ...
    try:                
        ...
        syslog.syslog("kojoney_tail.py : processdns() exception caught = " + repr(e) + "line=" + line)
...
def processAmunSyslog(line):
    ...
    try:
        ...
        syslog.syslog("kojoney_tail.py : processAmunSyslog() exception caught = " + repr(e) + "line=" + line)
...
def processMessages(line):
    ...
    try:
        ...
        syslog.syslog("kojoney_tail.py : processMessages() exception caught = " + repr(e) + "line=" + line)
...
def processHoneydSyslog(line):
    ...
    try:
        ...
        syslog.syslog("kojoney_tail.py : processHoneydSyslog() exception caught = " + repr(e) + "line=" + line)
...
def processFlows(line):
    ...
    try:            
        ...
        print("number of fields is " + repr(numFields))
        
        if numFields != 29:
            msg =  "error ! number of fields is " + repr(numFields) + ", expected number is 29"   
        ...
        if proto == "6" :	# TCP
            flagStr = netflowFlags(int(flags))
            flow2 = "F=" + flagStr + ",B=" + bytes + ",D=" + repr(duration_msecs) + ",P=" + pkts + ",AVG=" + repr(avg) + ",KBPS=" + "%.1f" % kbps + ",PPS=%.1f" % pps + ",inIf=" + inIf + ",outIf=" + outIf + ",min_plen=" + min_plen + ",max_plen=" + max_plen + ",minTTL=" + minTTL + ",maxTTL=" + maxTTL + ",ipid=" + IP_ID + ",rawPayload=" + payload + ",ASCIIpayload=" + decodedPayload
        else : 			# TCP flags are not relevent, so use "*" to denote this
            flow2 = "*"            + ",B=" + bytes + ",D=" + repr(duration_msecs) + ",P=" + pkts + ",AVG=" + repr(avg) + ",KBPS=" + "%.1f" % kbps + ",PPS=%.1f" % pps + ",inIf=" + inIf + ",outIf=" + outIf + ",min_plen=" + min_plen + ",max_plen=" + max_plen + ",minTTL=" + minTTL + ",maxTTL=" + maxTTL + ",ipid=" + IP_ID + ",rawPayload=" + payload + ",ASCIIpayload=" + decodedPayload
        ...
        if (srcIP == "HPOT") :	
            msg2 = rtr + ":" + "FLOW-OUT from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
        ...
        # MASTER for SEC processing #2
        if (dstIP == "HPOT") :	
            msg2 = rtr + ":" + "FLOW-IN from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps)
            logNetflowEvent(msg2)
            #syslog.syslog(msg2)
        ...
        if (srcIP == "HPOT" and int(duration_msecs) > 2000 and int(pkts) > 100 and (kbps > 2 or pps > 5) ) :	
            ...
            makeMsg(0,"0",msg)	
            msg2 = "FLOW,FLOOD-OUT from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) + " proto=" + proto + " src_port=" + srcPort + " flags=" + flags 
            logNetflowEvent(msg2)
            #sendTweet(msg)
            # send an email as well !!!
            #writeSecViz4(srcIP,srcPort,"172.31.0.67","22","EXPL:flw:SSH_session")
        ...
        # Case #7 : Malware retrieval using port 80 (HTTP) / 443 from external IP 
            ...
            makeMsg(0,"0",msg)
            msg = "FLOW,HTTP from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg)
            sendTweet(msg)
            ...
            makeMsg(0,"0",msg)
            msg = "FLOW,HTTPS from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg)
            sendTweet(msg)
            ...
            #msg = "flow," + rtr + ",REINF," + getIntelStr(srcIP,dstIP) + "," + flow1 + "," + "HTTPS_XFER" + "," + flow2
            #makeMsg(0,"0",msg)
            msg = "FLOW,FINGER from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg)
            sendTweet(msg)
        ...
        # Case #8 : Malware retrieval using port 21 (FTP) from external IP
            ...
            makeMsg(0,"0",msg)
            sendTweet(msg)
            msg = "FLOW,FTP from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg)
        ...
        # Case #9 : Malware retrieval using port 69 (TFTP) from external IP
            ...
            makeMsg(0,"0",msg)
            sendTweet(msg)
            msg = "FLOW,TFTP from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg)
        ...
        if decodedPayload.find("PING") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = rtr + ":" + "IRC_PING from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
        ...
        if decodedPayload.find("PONG") != -1  :
            ...
            msg2 = rtr + ":" + "IRC_PONG from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
            logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("USERHOST") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_USERHOST from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("NICK") != -1  :
            ...
            msg2 = rtr + ":" + "IRC_NICK from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
            logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("PRIVMSG") != -1  :
            ...
            print(msg)
            #msg2 = "FLOW-DPI,IRC_NICK from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + `duration_msecs`
            msg2 = rtr + ":" + "IRC_PRIVMSG from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
            logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("JOIN") != -1  :
            ...
            msg2 = rtr + ":" + "IRC_JOIN from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
            logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("Cannot join channel") != -1  :
            ...
            msg2 = rtr + ":" + "IRC_ERR_JOINFAIL from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
        ...
        if decodedPayload.find("Nickname is already in use") != -1  :
            ...
            msg2 = rtr + ":" + "IRC_ERR_BADNICK from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
        ...
        if decodedPayload.find("MODE") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_MODE from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("ERROR") != -1  :
            ...
            msg2 = rtr + ":" + "IRC_ERR from " + srcIP + " to " + dstIP + " proto=" + proto + " dp=" + dstPort + " sp=" + srcPort + " flags1=" + flags + " flags2=" + netflowFlags(int(flags)) + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs) + " kbps=" + repr(kbps) + " pps=" + repr(pps) 
        ...
        if decodedPayload.find("Ping timeout") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_ERR_PINGTIMEOUT from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("NOTICE AUTH") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_NOTICE_AUTH from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
            sendTweet(msg)
        ...
        if decodedPayload.find("NICK") != -1 and decodedPayload.find("USER") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_CREDENTIALS from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
        ...
        if decodedPayload.find("PART") != -1 and decodedPayload.find("JOIN") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_PART&JOIN from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
        ...
        if decodedPayload.find("AUTO") != -1 :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,IRC_AUTO from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #syslog.syslog(msg2)
            # Dump decode for later analysis
            msg2 = "FLOW-DPI,SPECIAL_INFO," + decodedPayload
            #logNetflowEvent(msg2)
        ...
        if decodedPayload.find("217 41 27 169") != -1 :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,MYIP from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
            # Dump decode for later analysis
            msg2 = "FLOW-DPI,SPECIAL_INFO," + decodedPayload
            #logNetflowEvent(msg2)
        ...
        if decodedPayload.find("SIP") != -1  :
            ...
            makeMsg(0,"0",msg)
            print(msg)
            msg2 = "FLOW-DPI,SIP from " + srcIP + " to " + dstIP + " on port " + dstPort + " bytes=" + bytes + " pkts=" + pkts + " msecs=" + repr(duration_msecs)
            #logNetflowEvent(msg2)
            sendTweet(msg)    
...
def processSebek(line):
    try:
        ...
        syslog.syslog("kojoney_tail.py : processSebek() exception caught = " + repr(e) + "line=" + line)
...
def icmpDecode(x):
    type = int(x/256)
    code = x % 256   
    icmpStr = "T=" + repr(type) + ":C=" + repr(code)
...
if pid == None:
    ...
    syslog.syslog("kojoney_tail.py started with pid " + repr(pid))
                
# Send an email to say kojoney_tail has started
now = time.time()
nowLocal = time.gmtime(now)
makeMsg(0,"0","system,kojoney_tail.py started with pid=" + repr(pid) + " at localtime " + time.asctime(nowLocal))
a = "kojoney_tail.py started with pid=" + repr(pid)
