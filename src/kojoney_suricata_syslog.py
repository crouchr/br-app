...
def processSur(line):
    try :
        ...
        doSyslog("kojoney_suricata_syslog.py : processSur() : exception caught = " + repr(e) + " line=" + line)
...
if pid == None:
    ...
    doSyslog("kojoney_suricata_syslog.py started with pid " + repr(pid))
...
a = "kojoney_suricata_syslog started with pid=" + repr(pid)
...
while (fileFound == False):
    try:
        ...
        print("fileSur = " + repr(fileSur))
        if fileSur != None:
            fileFound = True    
    except Exception as e:
        msg="kojoney_suricata_syslog.py : main() exception caught = " + repr(e)
        print(msg)
        doSyslog(msg)
        time.sleep(10)
